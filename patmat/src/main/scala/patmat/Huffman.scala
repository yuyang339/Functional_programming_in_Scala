...
object Huffman {
  ...
  // Part 1: Basics
    def weight(tree: CodeTree): Int = tree match {
      ...
      case f: Leaf => f.weight
    }// tree match ...
  
    def chars(tree: CodeTree): List[Char] = tree match{
      case f: Fork => f.chars
      case f: Leaf => f.char::Nil
    } // tree match ...
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def lookup(tree:  CodeTree)(c: Char): List[Bit] = tree match {
        case f: Leaf => List()
        case f: Fork => if (chars(f.left).contains(c)) 0 :: lookup(f.left)(c) else 1 :: lookup(f.right)(c)
      }
   ...
   */
    def convert(tree: CodeTree): CodeTable = {
      tree match {
        case f: Leaf => List((chars(f).head, List()))
